...
describe("httpBackClient", () => {
  beforeEach(() => {
    jest.spyOn(getConsole(), "error").mockImplementation(() => undefined);
    jest.spyOn(getConsole(), "info").mockImplementation(() => undefined);
    jest.spyOn(getConsole(), "log").mockImplementation(() => undefined);
  });
  ...
  it("throws HttpConnectionReset when fetch fails with ECONNRESET", async () => {
    jest.spyOn(global, "fetch").mockRejectedValue(new Error("read ECONNRESET"));
