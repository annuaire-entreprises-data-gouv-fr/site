...
const matomoHttpClient = createServerOnlyFn(async <T>(url: string) =>
  httpClient<T>({
    url,
...
export const clientMatomoStats = createServerOnlyFn(
  (): Promise<IMatomoStats> => store.get("matomoStats") as Promise<IMatomoStats>
);
