...
vi.mock("#/models/authentication/group/index.ts", () => ({
  getAgentGroups: vi.fn(),
  AgentsGroup: vi.fn(),
}));

vi.mock("#/models/authentication/agent/organisation");

const mockGetAgentGroups = vi.mocked(getAgentGroups);
const mockAgentOrganisation = vi.mocked(AgentOrganisation);
...
describe("AgentConnected", () => {
  ...
  describe("getHabilitationLevel", () => {
    ...
    it("should fallback to organisation habilitation when no agent habilitation", async () => {
      ...
      const mockGetHabilitationLevel = vi
        .fn()
