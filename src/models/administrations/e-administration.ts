export const EAdministration = {
  MAA: "maa",
  EDUCATION_NATIONALE: "education-nationale",
  AGENCE_BIO: "agence-bio",
  ADEME: "ademe",
  DILA: "dila",
  DINUM: "dinum",
  MC: "mc",
  MEF: "mef",
  MSS: "mss",
  INPI: "inpi",
  INSEE: "insee",
  MTPEI: "mtpei",
  MI: "mi",
  VIES: "vies",
  DJEPVA: "djepva",
  ESSFRANCE: "ess-france",
  MARCHE_INCLUSION: "marche-inclusion",
  INFOGREFFE: "infogreffe",
  DOUANES: "douanes",
  DGFIP: "dgfip",
  URSSAF: "urssaf",
  MSA: "msa",
  FNTP: "fntp",
  QUALIFELEC: "qualifelec",
  QUALIBAT: "qualibat",
  OPQIBI: "opqibi",
  CIBTP: "cibtp",
  CNETP: "cnetp",
  PROBTP: "probtp",
  GIP_MDS: "gip-mds",
  BANQUE_DE_FRANCE: "banque-de-france",
  CNIL: "cnil",
  CONSEIL_NATIONAL_DES_BARREAUX: "conseil-national-des-barreaux",
} as const;
export type EAdministration =
  (typeof EAdministration)[keyof typeof EAdministration];
