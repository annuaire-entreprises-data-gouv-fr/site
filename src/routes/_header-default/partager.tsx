...
export const Route = createFileRoute("/_header-default/partager")({
  head: () => {
    const canonical = "https://annuaire-entreprises.data.gouv.fr/partager";
    return {
      meta: meta({
  ...
  return (
    <TextWrapper>
      ...
      <div>
        ...
        <p>
          En ajoutant un lien cliquable depuis un site web (site officiel d’une
          entreprise, annuaire d’entreprises etc.), n’importe quel visiteur peut
          retrouver rapidement les informations essentielles de l’entreprise.
        </p>
...
></iframe>
        ...
        <p>
          ...
          <code>SIREN_OR_SIRET</code> avec le SIREN ou le SIRET pour lequel vous
          voulez créer un lien !
        </p>
        ...
        <ul>
          <li>
            Allez sur le <Link to="/">moteur de recherche</Link>
          </li>
        ...
        <br />
        <h2 id="link-search">Aider à la saisie de numéro de SIREN ou SIRET</h2>
        <h3>Quand l’utiliser ?</h3>
        <p>
          Si votre site internet, public ou privé, demande des informations
          liées à une personne morale dans un formulaire (par exemple, le SIRET
          ou le SIREN), vous pouvez fluidifier l’expérience utilisateur et
          limiter les erreurs de saisie, en invitant à retrouver les
          informations demandées grâce à{" "}
          <Link to="/">notre moteur de recherche</Link>.
        </p>
        <h3>Comment faire ?</h3>
        <p>
...
<a href="https://annuaire-entreprises.data.gouv.fr/">
  ...
  Entreprises
        ...
        <p>
          ...
          <Link to="/donnees/api">statut des API</Link> pour en savoir plus.
        </p>
      </div>
    </TextWrapper>
  );
