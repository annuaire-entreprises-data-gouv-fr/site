...
test.describe("Dirigeants ESSOR ENERGIES (SOLARSUD)", () => {
  test("Should display dirigeants from INPI", () => {
  ...
  test("[LOGGED] Should display comparison between INPI and IG", () => {
    cy.login();
    cy.visit("/dirigeants/487444697");
    cy.contains(
...
test.describe("Dirigeants non-diffusible ", () => {
  test("Should not display dirigeants", () => {
  ...
  test("[LOGGED] Should display dirigeants", () => {
    cy.login();
    cy.visit("/dirigeants/908595879");
