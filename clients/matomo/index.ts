...
export const getMatomoStats = async () => {
  try {
    const [matomoEventsResponse, matomoPagesResponse] = await Promise.all([
      fetch(
        `https://stats.data.gouv.fr/index.php?&expanded=1&format=JSON&idSite=${
          process.env.MATOMO_SITE_ID
        }&filter_limit=1999&method=Events.getName&module=API&&period=range&date=${getDateRange()}`
      ),
      fetch(
        `https://stats.data.gouv.fr/index.php?&expanded=1&format=JSON&idSite=${
          process.env.MATOMO_SITE_ID
        }&filter_limit=1999&method=Actions.getPageUrls&module=API&period=range&date=${getDateRange()}`
      ),
    ]);
