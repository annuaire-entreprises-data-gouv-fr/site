...
class AgentRateLimiting {
  private _agentRateLimitingStore: DataStore<IMonitoringAgent>;
  // time before rate limiting agents list update
  private TTL = 300000; // 5min

  constructor() {
    this._agentRateLimitingStore = new DataStore<IMonitoringAgent>(
      () => numberOfRequestByAgentList(),
  ...
  getMonitoringForAgent = async (email: string) => {
    try {
      return this._agentRateLimitingStore.get(email);
    } catch (e: any) {
      logFatalErrorInSentry(
        new FetchRessourceException({
          ressource: 'AgentRateLimiting',
          cause: e,
        })
      );
      return null;
...
export const agentRateLimiting = new AgentRateLimiting();
